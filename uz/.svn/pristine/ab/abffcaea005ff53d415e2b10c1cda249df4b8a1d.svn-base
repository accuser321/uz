	...
	art.dialog({
		...
		ok: function () {
    	  ...
    	  if(deptname.length==0){
    	    $("#depDiv .error").css({"display":"block"});
	        $('#depDiv .error span').html('请输入部门名称！');
	        return false;
    	  }
    	  if(upDept.length==0){
    	    $("#depDiv .error").css({"display":"block"});
	        $('#depDiv .error span').html('请选择员工所在上级部门！');
	        return false;
    	  }
  ...
  function modifyDepart(deptId){
    $.post("/departmentControll/searchDepartment.do",{deptId:deptId},function(msg){
        ...
        ok: function () {
            ...
            if(deptname.length==0){
              $("#depModfiyDiv .error").css({"display":"block"});
              $('#depModfiyDiv .error span').html('请输入要修改的部门名称！');
              return false;
            }
            if(upDept.length==0){
              $("#depModfiyDiv .error").css({"display":"block"});
              $('#depModfiyDiv .error span').html('请选择员工所在上级部门！');
              return false;
            }
