		...
		var singleUer = json_str.singleUer;
			...
			art.dialog({
				...
				ok:function (){
				    ...
				    var p2 = /^0?(13[0-9]|15[012356789]|17[0123456789]|18[0-9]|14[57])[0-9]{8}$/;
					if(phone.length==0){	      
				      $("#PopModify .error").css({"display":"block"});
				      $('#PopModify .error span').html('员工手机号码不能是空！');
				      return false;
					}else if(!(p2.test(phone))){
					  $("#PopModify .error").css({"display":"block"});
				      $('#PopModify .error span').html('您输入的手机号码格式不正确！');
				      return false;
					}
				    if(userSex.length==0){
				      $("#PopModify .error").css({"display":"block"});
